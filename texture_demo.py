...
# Whether to use an LLM or fall back to templates
USE_LLM = bool(os.getenv("OPENAI_API_KEY") or os.getenv("OPENONION_API_KEY"))
        ...
        try:
            code = llm_do(
                (
                    ...
                    f"- Import correctly\n"
                    f"- Cover public functions/classes with valid & invalid cases\n"
                    f"- No external IO/network; use mocks if needed\n"
...
def git_commit_push(branch: str = "auto/tests-docs") -> str:
    ...
    push = subprocess.run(
        ["git", "push", "-u", "origin", branch], cwd=ROOT, capture_output=True, text=True
    )
...
class IDEApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            with Vertical(id="left"):
                yield Static("Files in src/ (↑↓ to select, Enter to open)", id="title_l")
                tbl = DataTable(id="files"); tbl.add_columns("Path"); yield tbl
                yield Static("Keys: g=gen t=pytest d=diff y=apply p=push /=chat Esc=back", id="help")
            with Vertical(id="mid"):
                cv = CodeView(id="code"); yield cv
            with Vertical(id="right"):
                yield Static("Chat (/ to focus, Enter to send)", id="title_r")
                yield Chat(id="chat")
                yield Input(placeholder="Ask or command the agent", id="input")
        yield Footer()
    ...
    def on_mount(self):
        ...
        self.refresh_files()
        self.chat.append("Agent", "Hi! Select a file (Enter), press g to generate tests/docs, then t to run pytest.")
    ...
    def refresh_files(self):
        ...
        if not files:
            ...
            for f in files: self.table.add_row(f)
        self.table.cursor_type = "row"; self.table.focus()

    async def on_key(self, event):
        k = event.key.lower()
        if k == "/": self.input.focus()
        elif k == "escape": self.table.focus()
            ...
            self.open_file(rel)
        elif k == "g": await self.cmd_generate()
        elif k == "t": await self.cmd_test()
        elif k == "d": await self.cmd_propose_patch()
        elif k == "y": await self.cmd_apply_patch()
        elif k == "p": await self.cmd_commit_push()
    ...
    async def cmd_test(self):
        ...
        if ok:
            ...
            self.chat.append("Agent","❌ Tests failed. Tail shown in middle panel. Press d to propose a patch.")
    ...
    async def cmd_propose_patch(self):
        ...
        if not USE_LLM:
            self.chat.append("Agent","LLM disabled (no API key). Cannot auto-fix.")
            return
        self.chat.append("Agent","Analyzing failures, generating patch...")
        ...
        (ROOT/".last_patch.diff").write_text(patch, encoding="utf-8")
        self.chat.append("Agent","Patch displayed in center. Press y to apply it.")

    async def cmd_apply_patch(self):
        diff_file = ROOT/".last_patch.diff"
        if not diff_file.exists():
            self.chat.append("Agent","No patch found (.last_patch.diff missing).")
            return
        patch = diff_file.read_text(encoding="utf-8")
        ok,msg = git_apply_patch(patch)
        if ok:
            self.chat.append("Agent","✅ Patch applied. Run pytest again (t).")
        else:
